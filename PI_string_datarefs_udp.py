...
class PythonInterface:
    ...
    def FlightLoopCallback(self, elapsedMe, elapsedSim, counter, refcon):
        ...
        if len(fma_bytes) > 1472:
            print(self.Info, f"PI::FlightLoopCallback: returned value too large ({len(fma_bytes)}/1472)")
        else:
    ...
    def get_string_datarefs(self, acpath):
        ...
        if not os.path.exists(config_file):
            print(self.Info, f"PI::get_string_datarefs: Cockpitdecks config file '{config_file}' not found")
            return []
        with open(config_file, "r", encoding="utf-8") as config_fp:
            ...
            if self.trace:
                print(self.Info, f"PI::get_string_datarefs: Cockpitdecks config file '{config_file}' loaded, config length={len(ret)}, use default={self.use_defaults}.")
            return ret
