...
class FMAIcon(StringIcon):
    ...
    def get_image_for_icon(self):
        ...
        if not self.is_master_fma():
            ...
            return None
        image = Image.new(mode="RGBA", size=(FMA_COUNT * ICON_SIZE, ICON_SIZE), color=TRANSPARENT_PNG_COLOR)
        draw = ImageDraw.Draw(image)

        inside = round(0.02 * image.width + 0.5)
        ...
        for i in range(FMA_COUNT):
            ...
            for text in lines:
                ...
                if ref in self.boxed:
                    draw.rectangle(
                        [loffset + 2 * inside, h - text_size / 2] + [loffset + ICON_SIZE - 2 * inside, h + text_size / 2 + 4], outline=color, width=3
                    )
            loffset = loffset + ICON_SIZE
