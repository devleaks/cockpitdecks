...
def default_config(is_neo: bool = False, original: dict = {}):
    ...
    if len(original) > 0:
        for f in cfg.keys():
            ...
            if val is not None:
                cfg[f] = val
        logger.debug(f"merged default and original values")
    return cfg


def get_config_file(fn):
    data = {}
    with open(fn) as fp:
...
for livery in listdir(liveries):
    ...
    objects = join(d, "objects")  # A livery foder has a objects sub-folder
    if isdir(objects):
        is_neo = False
        data = {}
        ...
        if m is not None:
            ...
            if exists(cfg):  # Discrepencies magic string vs tlscfg.txt
                ...
                logger.debug(f"livery {livery}: has magic string only ({parsename(m)})")
